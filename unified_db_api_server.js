  ...
  postgresql: {
    ...
    async createCollection(collectionName) {
      try {
        ...
        console.error(
          `PostgreSQL createCollection error for ${collectionName}:`,
          err,
        );
        return false;
      }
    ...
    async collectionExists(collectionName) {
      try {
        ...
        console.error(
          `PostgreSQL collectionExists error for ${collectionName}:`,
          err,
        );
        return false;
      }
    ...
    async insertRecord(collectionName, data) {
      try {
        ...
        console.error(
          `PostgreSQL insertRecord error for ${collectionName}:`,
          err,
        );
        throw err;
      }
    ...
    async updateRecord(collectionName, id, data) {
      try {
        ...
        console.error(
          `PostgreSQL updateRecord error for ${collectionName}:`,
          err,
        );
        throw err;
      }
    ...
    async deleteRecord(collectionName, id) {
      try {
        ...
        console.error(
          `PostgreSQL deleteRecord error for ${collectionName}:`,
          err,
        );
        throw err;
      }
    ...
    async queryRecords(collectionName, query, options = {}) {
      try {
        ...
        if (query && Object.keys(query).length > 0) {
          ...
          for (const [key, value] of Object.entries(query)) {
            ...
            if (value === null) {
              ...
              conditions.push(
                `(data->'${key}')::numeric = $${queryParams.length}`,
              );
            } else if (typeof value === "boolean") {
              conditions.push(
                `(data->'${key}')::boolean = $${queryParams.length}`,
              );
            } else {
              queryParams.pop(); // Remove the value we just added
              conditions.push(
                `data->>'${key}' = '${JSON.stringify(value).replace(/'/g, "''")}'`,
              );
            }
          }
        ...
        console.error(
          `PostgreSQL queryRecords error for ${collectionName}:`,
          err,
        );
        throw err;
      }
  ...
  mongodb: {
    ...
    async connect() {
      try {
        ...
        const uri =
          process.env.MONGODB_URI || "mongodb://localhost:27017/unified_db_api";
    ...
    async createCollection(collectionName) {
      try {
        ...
        console.error(
          `MongoDB createCollection error for ${collectionName}:`,
          err,
        );
        return false;
      }
    ...
    async collectionExists(collectionName) {
      try {
        ...
        console.error(
          `MongoDB collectionExists error for ${collectionName}:`,
          err,
        );
        return false;
      }
...
async function initConnections() {
  ...
  if (process.env.DATABASE_URL) {
    ...
    console.log(
      "PostgreSQL connection string (DATABASE_URL) not provided. PostgreSQL support disabled.",
    );
  }
  ...
  if (process.env.MONGODB_URI) {
    ...
    console.log(
      "MongoDB connection string (MONGODB_URI) not provided. MongoDB support disabled.",
    );
  }

  if (!hasConnections) {
    console.warn(
      "No database connections established. API will have limited functionality.",
    );
  }
...
app.patch("/config", (req, res) => {
  ...
  if (!section || !updates || typeof updates !== "object") {
    return res
      .status(400)
      .json({ error: "Invalid request format. Requires section and updates." });
  }
...
app.post("/adapters", (req, res) => {
  ...
  if (!name || !type || !connectionConfig) {
    return res
      .status(400)
      .json({ error: "Missing required fields: name, type, connectionConfig" });
  }
  ...
  if (
    !config.supportedDatabases.sql.includes(name) &&
...
app.post("/connections", async (req, res) => {
  ...
  if (!name || !type || !connectionString) {
    return res
      .status(400)
      .json({ error: "Missing required fields: name, type, connectionString" });
  }
...
app.get("/connections", (req, res) => {
  const connections = Object.entries(dynamicConfig.customConnections).map(
    ([name, config]) => ({
      name,
      type: config.type,
...
app.post("/matrix", (req, res) => {
  ...
  if (!dimensions || !Array.isArray(dimensions)) {
    return res
      .status(400)
      .json({
        error: "Invalid dimensions configuration. Dimensions must be an array.",
      });
  }
...
app.post("/matrix/:name/value", (req, res) => {
  ...
  if (!coordinates || !Array.isArray(coordinates)) {
    return res
      .status(400)
      .json({ error: "Invalid coordinates. Coordinates must be an array." });
  }
...
app.get("/matrix/:name/value", (req, res) => {
  ...
  if (!coordinatesStr) {
    return res
      .status(400)
      .json({ error: "Coordinates query parameter is required" });
  }
...
app.post("/matrix/:name/query", (req, res) => {
  ...
  if (!query || typeof query !== "object") {
    return res
      .status(400)
      .json({ error: "Invalid query. Query must be an object." });
  }
...
app.post("/matrix/:name/dimensions", (req, res) => {
  ...
  if (!dimension || !dimension.name) {
    return res
      .status(400)
      .json({ error: "Invalid dimension. Dimension must have a name." });
  }
...
app.patch("/matrix/:name/resize", (req, res) => {
  ...
  if (!dimensions || !Array.isArray(dimensions)) {
    return res
      .status(400)
      .json({ error: "Invalid dimensions. Dimensions must be an array." });
  }
...
app.patch("/matrix/:name/storage", async (req, res) => {
  ...
  if (
    !storageType ||
    (storageType !== "memory" && storageType !== "database")
  ) {
    return res
      .status(400)
      .json({ error: 'Invalid storage type. Must be "memory" or "database".' });
  }
...
app.get("/databases", async (req, res) => {
  ...
  if (connections.postgresql.client) {
    ...
    databases.push({
      ...
      name: "postgres",
      collections: tables,
      status: (await connections.postgresql.testConnection())
        ? "connected"
        : "error",
    });
  }
  ...
  if (connections.mongodb.client) {
    ...
    databases.push({
      ...
      status: (await connections.mongodb.testConnection())
        ? "connected"
        : "error",
    });
  }
...
app.post("/databases/:dbType/:dbName/records", async (req, res) => {
  ...
  if (!connections[dbType] || !connections[dbType].client) {
    return res
      .status(400)
      .json({
        error: `Database type '${dbType}' is not supported or not connected`,
      });
  }
...
app.put("/databases/:dbType/:dbName/records/:id", async (req, res) => {
  ...
  try {
    const updated = await connections[dbType].updateRecord(
      collection,
      id,
      data,
    );
...
app.post("/databases/:dbType/:dbName/query", async (req, res) => {
  ...
  try {
    ...
    const records = await connections[dbType].queryRecords(
      collection,
      query,
      options,
    );
...
async function startServer() {
  ...
  app.listen(port, "0.0.0.0", () => {
    console.log(
      `Unified Database API Server running at http://0.0.0.0:${port}`,
    );
  });
}
