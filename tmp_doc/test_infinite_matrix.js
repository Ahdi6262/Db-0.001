...
async function runMatrixDemo() {
  ...
  try {
    ...
    console.log('Supported databases:', JSON.stringify(config.supportedDatabases));
    ...
    const setValueResponse = await client._fetch('/matrix/demo-3d-matrix/value', {
      method: 'POST',
      body: JSON.stringify({
    ...
    const value = await client._fetch('/matrix/demo-3d-matrix/value?coordinates=10,20,30');
    ...
    const addDimensionResponse = await client._fetch('/matrix/demo-3d-matrix/dimensions', {
      method: 'POST',
      body: JSON.stringify({
        name: 'time',
        type: 'numeric',
    ...
    console.log('First result:', queryResponse.results.length > 0 ? 
      queryResponse.results[0].value.data : 'No results');
    ...
    console.log('New query timeout:', updatedConfig.dynamic.performance.queryTimeout);
