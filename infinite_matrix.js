...
class InfiniteMatrix {
  ...
  set(coordinates, value) {
    ...
    if (coordinates.length !== this.dimensions.length) {
      throw new Error(
        `Expected ${this.dimensions.length} coordinates, got ${coordinates.length}`,
      );
    }
  ...
  addDimension(dimension) {
    ...
    const existingIndex = this.dimensions.findIndex(
  ...
  removeDimension(dimensionName) {
    const dimensionIndex = this.dimensions.findIndex(
  ...
  async changeStorage(storageType, options = {}) {
    ...
    // Save current data
    const currentData =
      this.storage.type === "memory" ? { ...this.data } : null;
